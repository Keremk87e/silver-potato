...
- Prisma + SQLite (Prisma 7 style config via `prisma.config.ts`)
...
## Prisma configuration note
- `prisma/schema.prisma` datasource no longer contains `url`.
- Connection URL for Prisma CLI/Migrate is defined in `prisma.config.ts`.
- Runtime Prisma Client is constructed with a SQLite adapter.
